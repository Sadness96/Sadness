...
namespace ADO.Helper.SQLite
{
    ...
    public class SQLiteHelper
    {
        ...
        /// SQLite 连接字符串
        /// </summary>
        private static string strSQLiteConnection { get; set; }
        ...
        {
            strSQLiteConnection = string.Format("data source={0};pooling=true;failifmissing=false", source);
        }
        ...
        public bool Open()
        {
            try
            {
                Connection = new SQLiteConnection(strSQLiteConnection);
                Connection.Open();
        ...
        public int CreateDataBase(string strDataSourcePath)
        {
            try
            {
                ...
                {
                    System.Data.SQLite.SQLiteConnection.CreateFile(strDataSourcePath);
                }
                else
                {
                    TXTHelper.Logs("SQLite数据库已存在,无法创建数据库,路径:" + strDataSourcePath);
                    return -1;
                }
            }
            catch (Exception ex)
        ...
        public List<string> GetAllTableName()
        {
            ...
            try
            {
                string sqlAllTable = string.Format("SELECT NAME FROM SQLITE_MASTER WHERE TYPE = 'table'");
                SQLiteDataAdapter DataAdapter = new SQLiteDataAdapter(sqlAllTable, Connection);
                if (Transaction != null)
                {
        ...
        public List<string> GetAllFieldName(string strTableName)
        {
            ...
            try
            {
                ...
                string sqlAllTable = string.Format("PRAGMA TABLE_INFO('{0}')", strTableName);
                SQLiteDataAdapter DataAdapter = new SQLiteDataAdapter(sqlAllTable, Connection);
                if (Transaction != null)
                {
                    DataAdapter.SelectCommand.Transaction = Transaction;
        ...
        public Dictionary<string, string> GetAllFieldNameType(string strTableName)
        {
            ...
            try
            {
                ...
                foreach (DataRow drTableName in dtAllTable.Rows)
                {
                    dicAllTableField.Add(drTableName["NAME"].ToString(), drTableName["TYPE"].ToString().ToUpper());
                }
            }
            catch (Exception ex)
            {
        ...
        public int ExecuteNonQuery(string sqlExecuteNonQuery)
        {
            ...
            try
            {
                ...
                SQLiteCommand Command = new SQLiteCommand(sqlExecuteNonQuery, Connection);
                if (Transaction != null)
                {
                    Command.Transaction = Transaction;
                }
        ...
        public DataSet GetDataSet(string sqlSelect)
        {
            ...
            try
            {
                ...
                SQLiteDataAdapter DataAdapter = new SQLiteDataAdapter(sqlSelect, Connection);
                if (Transaction != null)
                {
                    DataAdapter.SelectCommand.Transaction = Transaction;
                }
            ...
            try
            {
                ...
                foreach (DataRow drSourceData in dtSourceData.Rows)
                {
                    ...
                    foreach (var varFieldNameType in dicFieldNameType)
                    {
                        ...
                        if (strValue.IndexOf("BLOB") > -1 || string.IsNullOrEmpty(strValue) || !dtSourceData.Columns.Contains(strKei) || drSourceData.IsNull(strKei))
                        {
                            continue;
                        }
                        //拼写SQL,如果是数值型单独处理(不加'')
                        if (strValue.IndexOf("FLOAT") > -1 || strValue.IndexOf("DOUBLE") > -1 || strValue.IndexOf("REAL") > -1)
                        {
            ...
            try
            {
                ...
                foreach (DataRow drSourceData in dtSourceData.Rows)
                {
                    ...
                    foreach (var varFieldNameType in dicFieldNameType)
                    {
                        ...
                        //拼写SQL,如果是数值型单独处理(不加'')
                        else if (strValue.IndexOf("FLOAT") > -1 || strValue.IndexOf("DOUBLE") > -1 || strValue.IndexOf("REAL") > -1)
                        {
                            sqlFieldName += string.Format("{0},", strKei);
                            sqlFieldValue += string.Format("{0},", string.IsNullOrEmpty(drSourceData[strKei].ToString()) ? "null" : drSourceData[strKei].ToString());
                        }
                    ...
                    {
                        //带有二进制字段的数据每条数据保存一次
                        SQLiteCommand Command = new SQLiteCommand(sqlInsertInto, Connection);
                        foreach (string strByteDataFieldName in listByteDataFieldName)
                        {
                            ...
                            Command.Parameters.Add(strSqlFileName, DbType.Binary, byteFile.Length);
