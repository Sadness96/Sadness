...
namespace ADO.Helper.MySql
{
    ...
    public class MySqlHelper
    {
        ...
        /// MySql 连接字符串
        /// </summary>
        private static string strMySqlConnection { get; set; }
        ...
        public void MySqlConnectionString(string server, string id, string password, string database)
        {
            strMySqlConnection = string.Format("server={0};user id={1};password={2};database={3};port=3306;charset=utf8;", server, id, password, database);
        }
        ...
        public void MySqlConnectionString(string server, string id, string password, string database, string port)
        {
            strMySqlConnection = string.Format("server={0};user id={1};password={2};database={3};port={4};charset=utf8;", server, id, password, database, port);
        }
        ...
        public bool Open()
        {
            try
            {
                Connection = new MySqlConnection(strMySqlConnection);
                Connection.Open();
        ...
        public int CreateDataBase(string strDBName)
        {
            try
            {
                ...
                string sqlCreateDB = string.Format("CREATE DATABASE {0} DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;", strDBName);
                MySqlCommand Command = new MySqlCommand(sqlCreateDB, Connection);
                if (Transaction != null)
                {
        ...
        public int CreateTable(string strTableName, string strNameType)
        {
            try
            {
                ...
                MySqlCommand Command = new MySqlCommand(sqlCreateTable, Connection);
                if (Transaction != null)
                {
                    Command.Transaction = Transaction;
                }
        ...
        public List<string> GetAllDataBaseName()
        {
            ...
            try
            {
                string sqlAllTable = string.Format("SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA");
                MySqlDataAdapter DataAdapter = new MySqlDataAdapter(sqlAllTable, Connection);
                if (Transaction != null)
                {
                ...
                {
                    listAllTable.Add(drTableName["SCHEMA_NAME"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        public List<string> GetAllTableName()
        {
            ...
            try
            {
                string strDatabase = Connection.Database.ToString();
                if (string.IsNullOrEmpty(strDatabase)) return null;
                string sqlAllTable = string.Format("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{0}'", strDatabase);
                MySqlDataAdapter DataAdapter = new MySqlDataAdapter(sqlAllTable, Connection);
                if (Transaction != null)
                {
                    DataAdapter.SelectCommand.Transaction = Transaction;
        ...
        public List<string> GetAllFieldName(string strTableName)
        {
            ...
            {
                string strDatabase = Connection.Database.ToString();
                if (string.IsNullOrEmpty(strDatabase) || string.IsNullOrEmpty(strTableName)) return null;
                string sqlAllTable = string.Format("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '{0}' AND TABLE_NAME = '{1}'", strDatabase, strTableName);
                MySqlDataAdapter DataAdapter = new MySqlDataAdapter(sqlAllTable, Connection);
                if (Transaction != null)
                {
                    DataAdapter.SelectCommand.Transaction = Transaction;
        ...
        public Dictionary<string, string> GetAllFieldNameType(string strTableName)
        {
            ...
            try
            {
                string strDatabase = Connection.Database.ToString();
                if (string.IsNullOrEmpty(strDatabase) || string.IsNullOrEmpty(strTableName)) return null;
                string sqlAllTable = string.Format("SELECT COLUMN_NAME,COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '{0}' AND TABLE_NAME = '{1}'", strDatabase, strTableName);
                MySqlDataAdapter DataAdapter = new MySqlDataAdapter(sqlAllTable, Connection);
                if (Transaction != null)
                {
                    DataAdapter.SelectCommand.Transaction = Transaction;
                ...
                foreach (DataRow drTableName in dtAllTable.Rows)
                {
                    dicAllTableField.Add(drTableName["COLUMN_NAME"].ToString(), drTableName["COLUMN_TYPE"].ToString().ToUpper());
                }
            }
            catch (Exception ex)
            {
        ...
        public int ExecuteNonQuery(string sqlExecuteNonQuery)
        {
            ...
            try
            {
                ...
                MySqlCommand Command = new MySqlCommand(sqlExecuteNonQuery, Connection);
                if (Transaction != null)
                {
                    Command.Transaction = Transaction;
                }
        ...
        public DataSet GetDataSet(string sqlSelect)
        {
            ...
            try
            {
                ...
                MySqlDataAdapter DataAdapter = new MySqlDataAdapter(sqlSelect, Connection);
                if (Transaction != null)
                {
                    DataAdapter.SelectCommand.Transaction = Transaction;
                }
            ...
            try
            {
                ...
                foreach (DataRow drSourceData in dtSourceData.Rows)
                {
                    ...
                    foreach (var varFieldNameType in dicFieldNameType)
                    {
                        ...
                        if (strValue.IndexOf("BINARY") > -1 || strValue.IndexOf("BIT") > -1 || strValue.IndexOf("BLOB") > -1 || string.IsNullOrEmpty(strValue) || !dtSourceData.Columns.Contains(strKei) || drSourceData.IsNull(strKei))
                        {
                            continue;
                        }
                        //拼写SQL,如果是数值型单独处理(不加'')
                        if (strValue.IndexOf("INT") > -1 || strValue.IndexOf("FLOAT") > -1 || strValue.IndexOf("DOUBLE") > -1 || strValue.IndexOf("DECIMAL") > -1 || strValue.IndexOf("REAL") > -1)
                        {
            ...
            try
            {
                ...
                foreach (DataRow drSourceData in dtSourceData.Rows)
                {
                    ...
                    foreach (var varFieldNameType in dicFieldNameType)
                    {
                        ...
                        if (strValue.IndexOf("BINARY") > -1 || strValue.IndexOf("BIT") > -1 || strValue.IndexOf("BLOB") > -1)
                        {
                        ...
                        //拼写SQL,如果是数值型单独处理(不加'')
                        else if (strValue.IndexOf("INT") > -1 || strValue.IndexOf("FLOAT") > -1 || strValue.IndexOf("DOUBLE") > -1 || strValue.IndexOf("DECIMAL") > -1 || strValue.IndexOf("REAL") > -1)
                        {
                            sqlFieldName += string.Format("{0},", strKei);
                            sqlFieldValue += string.Format("{0},", string.IsNullOrEmpty(drSourceData[strKei].ToString()) ? "null" : drSourceData[strKei].ToString());
                        }
                    ...
                    {
                        //带有二进制字段的数据每条数据保存一次
                        MySqlCommand Command = new MySqlCommand(sqlInsertInto, Connection);
                        foreach (string strByteDataFieldName in listByteDataFieldName)
                        {
                            ...
                            Command.Parameters.Add(strSqlFileName, MySqlDbType.Binary, byteFile.Length);
